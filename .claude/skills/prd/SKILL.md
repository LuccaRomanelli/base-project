...
| ------------- | -------------------- | ---------------- |
| Badge         | components/ui/badge  | Status indicator |
| Dialog        | components/ui/dialog | Edit modal       |
| useOptimistic | hooks/               | Instant updates  |

### Relevant Existing Code

- `src/app/tasks/actions.ts` - Task mutations (extend for new feature)
- `src/components/TaskCard.tsx` - Add new display here
...
For each user story, specify `testType` (unit|integration|e2e|none).
...
- **Extend:** schema, actions.ts, TaskCard.tsx
