...
if [ ! -f "$SCRIPT_DIR/RALPH.md" ]; then
  echo "Error: RALPH.md not found at $SCRIPT_DIR/RALPH.md"
  exit 1
fi
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  while [[ $RETRY_COUNT -lt $MAX_RETRIES ]]; do
    OUTPUT=$(claude --dangerously-skip-permissions --print < "$SCRIPT_DIR/RALPH.md" 2>&1 | tee /dev/stderr) && break || EXIT_CODE=$?
